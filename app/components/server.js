...
const API_URL_FILMS = 'https://kinopoiskapiunofficial.tech/api/v2.2/films?countries=5&order=RATING&type=FILM&ratingFrom=6&ratingTo=10&yearFrom=2017&yearTo=2023&page=1';
const API_URL_RELEASES = 'https://kinopoiskapiunofficial.tech/api/v2.1/films/releases?year=2023&month=DECEMBER&page=1';
...
function showTrendsMovies(data) {
  ...
  data.items.forEach((movie, index) => {
    if (index < 3) {
      const genreString = `${movie.genres.map((genre, index) => index < 2 ? ` ${genre.genre}` : '')}`;

      movieElement.innerHTML += `
        <li class="film" style="background-image: url('${movie.posterUrlPreview}');">
          <span class="film_gradient"></span>

          <div class="film_info">
            <span>
              <p class="film_name">${movie.nameOriginal == null || movie.nameOriginal == "" ? `${movie.nameRu}` : `${movie.nameOriginal}`}</p>
              <p class="film_genre">${genreString.split(',').join(' ').trim()} | <span class="film_year">${movie.year}</span></p>
            </span>
            <ul class="starbar">
